...
and vice-versa.
Note that the \textit{source} URL of this \tcode{<firebase-element>}\index{<firebase-element>} 
is itself a \tcode{\{\{ \}\}} bound variable (\tcode{location}) which finds the location with the \tcode{fbLocation} method.
...
A handler in the top level container (\tcode{<speakur-discussion>}\index{<speakur-discussion>}) responds to this named event (\tcode{speakur-locale-change}) and tells the internationalization component to update the language.
...
As mentioned above, the custom element itself is the `model' and its \tcode{<template>} is the view.
...
\begin{enumerate}
\item The \tcode{<template>} binds to \tcode{canEditPost} to determine if the \tcode{<div>} containing the Edit link should be shown on a post. I.e, is this user allowed to edit this post.
\item The \tcode{<div>} binds its click event to \tcode{editPost} so it functions as a link. The \tcode{editPost} method will be called when this is clicked.
\item The inner content of the \tcode{<div>} (its light DOM) is bound to a Polymer filter expression that translates ``Edit'' to the current language. 
\end{enumerate}
...
That said, keeping \tcode{<template>} expressions simple and moving more complex logic to methods in the JavaScript controller helps uphold separation of concerns between controller and view.

\section{Security}
\label{sec:security}
Because Firebase\index{Firebase} is Speakur's only `server', 
most of its security\index{security} rests in Firebase's authentication and authorization rules.
...
and the Firebase\index{Firebase} security rules enforce authorization requirements.

\subsection{Authentication}
Speakur\index{Speakur} currently supports signing in users (authenticating\index{authentication}) with Google\index{Google} and Facebook\index{Facebook} profiles.
...
The user authenticates directly to the chosen IdP (e.g. Facebook\index{Facebook}) and their password is never sent to Firebase or Speakur.
...
The authentication\index{authentication} session is automatically used by any \tcode{<firebase-element>} anywhere on the page provided their \tcode{location=} points to the same database URL.

\subsection{Firebase Security Policy Rules}
\label{sec:fbsecurity}

Authorization to perform any given action in the Speakur database is determined by consulting the nested rules in the 
...
The current translations were obtained by taking the English language file and passing it through the Microsoft\index{Microsoft} Translation API\index{API} using `Eurgh!'\index{Eurgh},
a Python\index{Python} package that I wrote for another project.~\cite{landers2015-b}.
...
An example of interpolating two named variables (\tcode{apples} and \tcode{oranges}) is provided, 
...
\tcode{i18next}\index{i18next library} sees that a \tcode{count} variable is provided to \tcode{\$\$} and looks for the key (replies) with either a \tcode{\_count} or a \tcode{\_count\_plural} suffix depending on the cardinality of \tcode{count}.
The \tcode{lc} variable reflects the user's current language (locale) setting, such as \tcode{lc=en} for English.
Interestingly, the \tcode{lc} variable is not required by the \tcode{\$\$} filter at this point,
...
Another important localization feature is provided by the moment.js library\index{moment.js library}, 
...
Speakur published to \tcode{github.io}, 
