...
\begin{lstlisting}[language=JavaScript,numbers=none,caption=
...
To understand the general flow of information in Speakur (and Polymer\index{Polymer}), imagine the component as an inverted tree, with the `root' at the top 
...
DOM\index{DOM} mutation observers\index{mutation observers} are a standard way to register a callback to be run when some area of the page (DOM) changes. 
...
which are not subject to Same-Origin Policy\index{Same-Origin Policy} restrictions.
