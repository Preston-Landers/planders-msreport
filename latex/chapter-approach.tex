...
The extensive use of CSS flex\index{Flex} box attributes ensures that structural elements like headers and toolbars automatically adjust their layout to available space. 
...
Updating an existing post's text is done with \tcode{POST}\index{HTTP!POST}.
...
and its metadata\index{metadata} to be addressed as a set of linked HTTP resources.
...
Confidentiality of in-transit data is handled with the same Transport Layer Security (TLS)\index{transport layer security (TLS)}, 
...
Speakur's\index{Speakur} actual rules are described in greater detail in section~\ref{sec:security}.
...
and the nodes and leaves under that being the internal parts such as the headers, the posts, and other user interface\index{user interface (UI)} controls.
...
The parent (or any other interested party) can register a DOM\index{DOM} mutation observer to be notified when the child value changes, or the child can fire an \textit{events}\index{event notification} that the parent can respond to.
...
which are not subject to Same-Origin Policy\index{Same-Origin Policy} restrictions on asynchronous JavaScript HTTP\index{HTTP} requests (also known as AJAX\index{AJAX} or XMLHttpRequest\index{XMLHttpRequest}).
...
Of course, that is really a \textit{feature} designed to enhance code organization, encapsulation\index{encapsulation} and productivity, 
...
Techniques like \tcode{Keep-Alive} headers can help, and the forthcoming HTTP 2.0\index{HTTP!HTTP 2.0} will address this question more comprehensively.
...
single\footnote{Except when Content Security Policy (CSP)\index{content security policy (CSP)} requires enforced separation of HTML, CSS, and JS. In this case, three files are used.}
