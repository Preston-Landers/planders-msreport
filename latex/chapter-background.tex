...
Custom HTML Elements and Shadow DOM\index{Shadow DOM|textbf} are the two key players while HTML Imports\index{HTML!Imports} and Templates\index{HTML!Templates} support these features. 
...
HTML5 Shadow~DOM\index{Shadow DOM|textbf} should not be confused with the React\index{React} framework's \textbf{Virtual}~DOM, which is conceptually closer to HTML5 Templates in nature than Shadow DOM.}. 
...
Careful design in the library can minimize that impact, but native browser support offers a more consistent approach.
...
Flex boxes are intended to replace the use of the \tcode{<table>}\index{<table>} tag for formatting and structure, as opposed to actual tables of information~\cite{mozillacontributors2015}.
...
localization\index{localization} features for a global audience.\label{motive:i18n}
