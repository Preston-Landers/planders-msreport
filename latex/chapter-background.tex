...
Creating your own \textit{sui generis} HTML elements with unique behaviors was beyond the capabilities of the web browsers of the day like Mosaic\index{Mosaic}
and Netscape Navigator\index{Netscape Navigator}.

As of early 2015, modern web apps are typically written with a JavaScript\index{JavaScript} framework that provides a cohesive set of structures, design patterns and practices designed to facilitate composing web applications---large or small---from a number of sub-com\-ponents~\cite{dickey2014}.
...
Yet all frameworks must exist within the confines of the programming model provided by the browser and the Document Object Model\index{DOM}. 
In the DOM, the entire web page or app belongs to a single `document', constituent parts are not encapsulated\index{encapsulation} or isolated from each other, and authors are limited to working with the predefined HTML tags.
...
Throughout this report I will refer to both Web Components (the W3C initiative) and a more general concept of a ``software component'', 
or just component for short, 
which can be anything from a short function to a major software package or service.
I also refer to ``widgets'', which are user interface (UI)\index{user interface (UI)} components like menus and buttons.

\section{Current challenges in web authoring}
In object oriented programming (OOP)\index{object oriented programming (OOP)}, encapsulation\index{encapsulation} is typically defined as a 
...
\end{quote}
Among the key benefits of OOP are inheritance and polymorphism\index{polymorphism}, or the ability to treat related types of objects the same way through a shared interface.

Although techniques of abstraction\index{abstraction} and encapsulation\index{encapsulation} have been widespread in object oriented programming\index{object oriented programming (OOP)} for decades,
the fundamental web client programming model has not allowed for significant encapsulation of things like the DOM structure and CSS style rules or polymorphic treatment of different components~\cite{ihrig2012}.
To illustrate how these problems affect the ability of authors to share and reuse code, 
let us look at an example from the popular Twitter\index{Twitter} Bootstrap\index{Bootstrap} library~\cite{bootstrapcontributors2015}.
Twitter Bootstrap is a collection of Cascading Style Sheet (CSS)\index{CSS} rules and JavaScript\index{JavaScript} user interface components designed to allow web authors to quickly `bootstrap' an attractive, consistent look-and-feel onto a web page.
...
\begin{lstlisting}[language=JavaScript,caption={Placing a custom element\index{Custom Elements} imperatively in JavaScript\index{JavaScript}.},label=l:register]
...
leaving only Polymer\index{Polymer} and the less-comp\-rehen\-sive X-Tags\index{X-Tags}~\cite{x-tagscontributors2015} and Bosonic~\cite{bosoniccontributors2014}\index{Bosonic} projects and a few other less ambitious contenders.
