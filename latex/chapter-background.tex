...
This coupling hinders a significant refactoring of the navigation widget's internal structure because that would require the large community of developers to update their applications accordingly.
...
like Python\index{Python}, Go\index{Go} and Ruby\index{Ruby}. 
...
This aids performance and user experience (UX)\index{UX} by ensuring that the callback\index{callback} is only called ``as-needed'' rather than once for each small change.
...
A team within Google\index{Google} has developed the 
Polymer\index{Polymer} framework based on Web Components\index{Web Components} 
architecture~\cite{polymercontributors2015}.
